...
- `state/cache`: JPX、Yahoo Finance、トレーダーズ・ウェブの取得結果を保存
...
標準cronはTDnetをJST平日08:03から19:53まで10分間隔、日次処理を05:37に設定しています。寄り付き向け通知は06:03から08:43まで20分ごとに再試行し、`not_before_jst`を満たした最初の実行だけが送信します。GitHub Actionsはscheduled eventの実行時刻を保証しないため、長い事前バッファと複数回の再試行を使います。
...
- トレーダーズ・ウェブ株式分割一覧: PDFで取得できない権利付最終日と分割比率を確定補完
...
POの対象判定は80億円以上です。価格決定前は「発表日終値×（公募株数＋売出株数＋OA株数）」、価格決定後は決定価格と株数内訳から再計算します。一度80億円以上になった案件は後から下回っても対象を維持します。`#po` は対象確定時のPO予定通知と、価格決定日・+1・+2・受渡日・+25・+26営業日の予定通知だけを送信します。
