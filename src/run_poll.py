...
from src.core.store import add_notified_id, find_events, has_notified, load_state, save_state, upsert_event
...
def main(argv: list[str] | None = None) -> int:
    ...
    same_day_buybacks = {(item.code, item.announced_at.date().isoformat()) for item in disclosures if "buyback" in classify_title(item.title)}
    ...
    for disclosure in disclosures:
        ...
        try:
            if "buyback" in classes:
                ...
                changed |= add_notified_id(state, disclosure.id)
                continue

            if "po_pricing" in classes:
                changed |= handle_po_pricing(disclosure, state)
                changed |= add_notified_id(state, disclosure.id)
                continue
            ...
            if "cb" in classes:
                changed |= handle_cb(disclosure, state, notifier, master, margin, same_day_buybacks)
                changed |= add_notified_id(state, disclosure.id)
                continue

            if "split" in classes:
                changed |= handle_split(disclosure, state, master, margin)
                changed |= add_notified_id(state, disclosure.id)
        except Exception as exc:
            notifier.system(f"TDnet処理失敗: {disclosure.code} {disclosure.title}: {exc}")
            raise
...
def handle_po(disclosure: Disclosure, state: dict[str, Any], notifier: SlackNotifier, master: dict[str, Any], margin: dict[str, str]) -> bool:
    ...
    stored, changed = upsert_event(state, event)
    notifier.send("po", format_po_announcement(stored), header="PO発表", pdf_url=stored.get("pdf_url"))
    return True


def handle_po_pricing(disclosure: Disclosure, state: dict[str, Any]) -> bool:
...
def handle_bunbai(disclosure: Disclosure, state: dict[str, Any], notifier: SlackNotifier, master: dict[str, Any], margin: dict[str, str]) -> bool:
    ...
    event["detail"] = {"execution_date": None, "execution_date_raw": None}
    stored, changed = upsert_event(state, event)
    notifier.send("bunbai", format_bunbai_announcement(stored), header="立会外分売発表", pdf_url=stored.get("pdf_url"))
    return True
...
def handle_cb(
    ...
    stored, changed = upsert_event(state, event)
    notifier.send("cb", format_cb_announcement(stored), header="CB発表", pdf_url=stored.get("pdf_url"))
    return True


def handle_split(disclosure: Disclosure, state: dict[str, Any], master: dict[str, Any], margin: dict[str, str]) -> bool:
    text = fetch_pdf_text(disclosure.pdf_url)
